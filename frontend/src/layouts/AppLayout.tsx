...
const navItems = [
  ...
  {
    to: '/agents-mgmt',
    labelKey: 'nav.agents_mgmt',
    icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z',
  },
  {
    to: '/agent-skills',
    labelKey: 'nav.agent_skills',
  ...
  { to: '/triggers', labelKey: 'nav.triggers', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
  {
    to: '/settings',
...
export function AppLayout() {
  ...
  const breadcrumbMap: Record<string, string> = {
    ...
    '/model-settings': t('nav.model_settings'),
    ...
    '/workflows/template': t('nav.workflows'),
    '/workflows/instances': t('nav.workflows'),
    '/workflows/instance': t('nav.workflows'),
    '/workflows/reviews': t('nav.workflows'),
  ...
  const currentPageLabel = (() => {
    ...
    const mapped = breadcrumbMap[p];
    if (mapped) return mapped;
    if (p.startsWith('/workflows/template/')) return t('breadcrumb.workflow_template');
    if (p.startsWith('/workflows/instance/')) return t('breadcrumb.workflow_instance');
    return p;
  })();
  const breadcrumbItems =
    location.pathname === '/'
      ? [{ label: breadcrumbMap['/'] }]
      : [{ label: breadcrumbMap['/'] || 'Home', path: '/' }, { label: currentPageLabel }];
          ...
          >
            ...
            {!collapsed && (
              <span style={{ fontSize: 'var(--text-xs)' }}>
                {connState === 'connected'
                  ? t('gateway.state_connected')
                  : connState === 'connecting'
                    ? t('gateway.state_connecting')
                    : t('gateway.state_disconnected')}
              </span>
            )}
          </div>
          <Dropdown
