...
export const templatesApi = {
  ...
  list: (params?: TemplateListParams) => {
    ...
    return apiGet<PaginatedResponse<WorkflowTemplateListItem>>(`${BASE}${query ? `?${query}` : ''}`)
  },
  ...
  get: (templateId: string) => 
    apiGet<WorkflowTemplate>(`${BASE}/${templateId}`),

  /**
   * 创建模板
   */
  create: (data: CreateTemplateRequest) => 
    apiPost<WorkflowTemplate>(BASE, data),
  ...
  delete: (templateId: string) => 
    apiDelete(`${BASE}/${templateId}`),

  /**
   * 发布模板
   */
  publish: (templateId: string) => 
    apiPost<WorkflowTemplate>(`${BASE}/${templateId}/publish`, {}),

  /**
   * 归档模板
   */
  archive: (templateId: string) => 
    apiPost<WorkflowTemplate>(`${BASE}/${templateId}/archive`, {}),
